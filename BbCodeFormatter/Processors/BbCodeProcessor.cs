...
namespace BbCodeFormatter
{
  ...
  /// </summary>
public class BbCodeProcessor : IRequiresSessionState
{
    #region  Private Class Member Declarations

    private static bool _useFullUrl;
    static List<IHtmlFormatter> _formatters;
    static List<IHtmlFormatter> _postformatters;
    static List<IHtmlFormatter> _cleancodeformatters;
    static List<IHtmlFormatter> _urlformatters;
    static List<IHtmlFormatter> _codeformatter;
    static List<IHtmlFormatter> _quoteformatter;
    static List<IHtmlFormatter> _tables;
    ...
    static BbCodeProcessor()
    {
        ...
        _urlformatters.Add(new UrlFormatter(ClassicConfig.ForumUrl + @"(?![?])\S+(TOPIC_ID=)([0-9]+)(#*[0-9]*)", Config.ForumUrl + "Topic/Posts/$2?pagenum=-1$3"));
        _urlformatters.Add(new UrlFormatter(Config.ForumUrl + @"(?![?])\S+(TOPIC_ID=)([0-9]+)(#*[0-9]*)", Config.ForumUrl + "Topic/Posts/$2?pagenum=-1$3"));
        _urlformatters.Add(new UrlFormatter(ClassicConfig.ForumUrl + @"(?![?])\S+(FORUM_ID=)([0-9]+)", Config.ForumUrl + "Forum/Posts/$2"));
        _urlformatters.Add(new UrlFormatter(Config.ForumUrl + @"(?![?])\S+(FORUM_ID=)([0-9]+)", Config.ForumUrl + "Forum/Posts/$2"));
        _urlformatters.Add(new UrlFormatter(ClassicConfig.ForumUrl + "search.asp", Config.ForumUrl + "Forum/Search"));
        _urlformatters.Add(new UrlFormatter(Config.ForumUrl + "search.asp", Config.ForumUrl + "Forum/Search"));
        _urlformatters.Add(new UrlFormatter(ClassicConfig.ForumUrl + "faq.asp", Config.ForumUrl + "Help"));
    ...
    if (ClassicConfig.GetValue("STRIMGINPOSTS") == "1")
    {
        _formatters.Add(new RegexFormatter(@"(?<!"")\[img(?:\s*)\]((.|\n)*?)\[/img(?:\s*)\](?!"")", "<img class=\"lazyload\" data-src=\"$1\" border=\"0\" alt=\"\" />"));
        _formatters.Add(new RegexFormatter(@"\[img align=((.|\n)*?)(?:\s*)\]((.|\n)*?)\[/img(?:\s*)\]", "<img data-src=\"$3\" border=\"0\" class=\"lazyload img-$1\" alt=\"\" />"));
        _formatters.Add(new RegexFormatter(@"\[img=((.|\n)*?)((?:[x])((.|\n)*?))*(?:\s*)\]((.|\n)*?)\[/img(?:\s*)\]", "<img class=\"lazyload\" width=\"$1\" height=\"$4\" data-src=\"$6\" border=\"0\" alt=\"\" />"));
        _formatters.Add(new RegexFormatter(@"\[img=((.|\n)*?)\]((.|\n)*?)\[/img=((.|\n)*?)\]", "<img data-src=\"$3\" border=\"0\" class=\"lazylod img-$1\" alt=\"\" />"));
        //Config.ForumUrl
    ...
    if(ClassicConfig.GetIntValue("STRICONS")==1)
        _formatters.Add(new EmoticonFormatter());
    ...
    #region PostFormat
        ...
        {
            _postformatters.Add(new BadWordFilter());
        }
    ...
    public static string Format(string data,bool parseurls=true, bool tooltip = false, bool newsfeed = false)
    {
        _useFullUrl = newsfeed;
    if (ClassicConfig.GetValue("STRALLOWFORUMCODE") != "1")
        return data;
    ...
    if (ClassicConfig.GetIntValue("STRPHOTOALBUM") == 1)
    {
        if (_useFullUrl)
        {
            data = Regex.Replace(data, @"\[image=(?<id>\d+)]", "<a href=\"" + Config.ForumUrl + "PhotoAlbum/GetPhoto/${id}\" class=\"view-image\" target=\"_blank\"><img class=\"lazyload\" data-src=\"" + Config.ForumUrl + "PhotoAlbum/Thumbnail/${id}\" border=\"0\" /></a>", RegexOptions.IgnoreCase);
        }
        else
        {
            data = Regex.Replace(data, @"\[image=(?<id>\d+)]", "<a href=\"" + Common.RootFolder + "/PhotoAlbum/GetPhoto/${id}\" class=\"view-image\" target=\"_blank\"><img class=\"lazyload\" data-src=\"" + Common.RootFolder + "/PhotoAlbum/Thumbnail/${id}\" border=\"0\" /></a>", RegexOptions.IgnoreCase);
        }

        var imgf = new AlbumImageFormatter(@"\[cimage=(?<id>\d+)]");
        data = imgf.Format(data);
    }

    if (Config.TableExists("FORUM_BBCODE"))
    {
        data = CustomCode(data);
    }
    //else if (Config.TableExists("FORUM_BBCODE",new HttpContextWrapper(HttpContext.Current)))
    //{
    //    data = CustomCode(data);
    //}
    ...
    if (Config.ContentFolder != "Content")
    {
        ...
        if (ClassicConfig.GetIntValue("INTPROTECTPHOTO") == 1)
        {
    ...
    public static string CleanCode(string data)
    {
        if (String.IsNullOrWhiteSpace(data))
            return data;
    ...
    public static string Subject(string data)
    {
        if (String.IsNullOrWhiteSpace(data))
            return data;
    ...
    /// <param name="data"></param>
    /// <returns></returns>
    public static string StripCodeContents(string data)
    {
        var removecode = new List<IHtmlFormatter>
        {
            new SearchReplaceFormatter("[scrollcode][code]", "[scrollcode]"),
    ...
    /// <param name="data"></param>
    /// <returns></returns>
    private static string NoParse(string data)
    {
        if (String.IsNullOrWhiteSpace(data))
            return data;
        const RegexOptions options = RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline;
    ...
    /// <returns></returns>
    private static string CustomCode(string data)
    {
        if (String.IsNullOrWhiteSpace(data))
            return data;
