...
var Grid = fc.Grid = RowRenderer.extend({
	...
	constructor: function() {
		...
		this.isRTL = arguments['isRTL'];
        this.isJalaali = arguments['isJalaali'];
  ...
  //          '</th>';
        return '' +
            '<th class="fc-day-header ' + view.widgetHeaderClass + ' fc-' + dayIDs[date.day()] + '"' +
            (this.rowCnt == 1 ? ' data-date="' + date.format('YYYY-MM-DD', view.isJalaali) + '"' : '') +
            '>' +
            htmlEscape(date.format(this.colHeadFormat, view.isJalaali)) +
            '</th>';
	},
	...
	getEventTimeText: function(range, formatStr, displayEnd) {
		...
		if (this.displayEventTime && range.start.hasTime()) {
            if (displayEnd && range.end) {
                return this.view.formatRange(range, formatStr, this.view.isRTL, this.view.isJalaali);
			}
            else {
                return range.start.format(formatStr, this.view.isJalaali);
			}
		}
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (seg.isStart) {
            ...
            if (timeText) {
                timeHtml = '<span class="fc-time"' + ' data-start="' + event.start.format(view.isJalaali) + '"' + '>' + htmlEscape(timeText) + '</span>';
			}
		}
...
Calendar.defaults = {
	...
	buttonIcons: {
        prev: this.isRTL ? 'right-single-arrow' : 'left-single-arrow',
        next: this.isRTL ? 'left-single-arrow' : 'right-single-arrow',
        prevYear: this.isRTL ? 'right-double-arrow' : 'left-double-arrow',
        nextYear: this.isRTL ? 'left-double-arrow' : 'right-double-arrow'
	},
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($('<h5>&nbsp;</h5>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
...
var MonthGrid = DayGrid.extend({
	...
	monthCellHtml: function(cell) {
		...
		return '<td class="' + classes.join(' ') + '"' +
			' data-date="' + date.format('YYYY-MM-DD',view.isJalaali) + '"' + // if date has a time, won't format it
				'></td>';
	},
	...
	monthNameCellHtml: function(cell) {
		...
		return '' +
			'<th class="' + classes.join(' ') + '" data-month="' + date.month() + '" data-date="' + date.format(this.view.isJalaali) + '">' +
            date.format(this.colHeadFormat, this.view.isJalaali) +
					'</th>';
	},
	...
	dateToCellOffset: function(date) {
		...
		if (month < 0) {
			...
			return offsets[Math.ceil(month)];
		}
	},
...
/* A year view with month cells running in rows and columns
----------------------------------------------------------------------------------------------------------------------*/
...
var YearView = fcViews.year = BasicView.extend({ // make a subclass of View
    ...
    computeRange: function (date) {
        var range = View.prototype.computeRange.call(this, date); // get value from the super-method
        range.start._d.setMonth(0);
        range.start._d.setDate(1);
        range.end._d.setMonth(11);
        range.end._d.setDate(31);
        range.end._d.setYear(range.start.year());
        return range;
	},
	// Renders the view into `this.el`, which should already be assigned
	render: function() {
...
var AgendaView = fcViews.agenda = View.extend({
	...
	setRange: function(range) {
		View.prototype.setRange.call(this, range); // call the super-method
        this.timeGrid.breakOnWeeks = /year|month|week/.test(this.intervalUnit); // do before setRange
...
;;
    ...
    var ListView = fcViews.list = View.extend({
        ...
        render: function () {
            this.el.html(this.renderHtml());
            this.dayNumbersVisible = false; // TODO: make grid responsible
        ...
        computeTitle: function () {
            return this.formatRange(
                { start: this.intervalStart, end: this.end },
                this.opt('titleFormat') || this.computeTitleFormat(),
                this.opt('titleRangeSeparator')
            );
        },
        ...
        renderEvents: function (events) {
            ...
            while (currentDayStart.isBefore(periodEnd)) {
                ...
                for (var i = eventsCopy.length - 1; i >= 0; --i) {
                    ...
                    if (currentDayStart.isAfter(eventEnd) || (currentDayStart.isSame(eventEnd,'day') && !eventStart.isSame(eventEnd,'day')) || periodEnd.isBefore(eventStart)) {
                        ...
                        if (!didAddDayHeader) {
                            tbody.append('\
                                <li class="my-fc-header" date="">\
                                    <span class="my-fc-header-day">' + currentDayStart.format('dddd', this.isJalaali) + '</span>\
                                    <span class="my-fc-header-date">' + (this.isJalaali ? currentDayStart.format('YYYY-MM-DD', this.isJalaali) : currentDayStart.format(this.opt('dateFormat'))) + '</span>\
                                </li>');
                            //(this.isJalaali  ? currentDayStart.format('YYYY-MM-DD', this.isJalaali) :  currentDayStart.format(this.opt('dateFormat')))
                            //.format(this.opt('dateFormat'), this.isJalaali)
                            //currentDayStart.format('YYYY-MM-DD', this.isJalaali)
                            didAddDayHeader = true;
                        }
                        var userlink = e.title;
                        if (e.className == 'event-birthday') {
                            userlink = '<a href="' + SnitzVars.baseUrl + 'Account/UserProfile/' + e.title.trim() + '">' + e.title + '</a>';
                        }
                        ...
                        var segEl = $('\
                            <li class="my-fc-row ' + e.className + '">\
                                ...
                                <div class="my-fc-title" style="color: rgb(55, 90, 127); font-weight: bold">--' + userlink + '--</div>\
                            </li>');
                        tbody.append(segEl);
    ...
    ;;
    /*---------------------------------------------------- CALLIST VIEW -----------------------------------------------------*/
    var CalListView = fcViews.callist = View.extend({
        ...
        renderEvents: function (events) {
            ...
            while (currentDayStart.isBefore(periodEnd)) {
                ...
                for (var i = eventsCopy.length - 1; i >= 0; --i) {
                    ...
                    var postdate = this.calendar.moment(e.posted);
                    ...
                    if (currentDayStart.isAfter(eventEnd) || (currentDayStart.isSame(eventEnd) && !eventStart.isSame(eventEnd)) || periodEnd.isBefore(eventStart)) {
                        ...
                        if (!didAddDayHeader) {
                            ...
                            if (e.author === e.currentuser) {
                                segTel += '<a href="' + SnitzVars.baseUrl + 'Events/AddEditEvent/' + e.id + '"><i class="fa fa-edit fa-1_5x"></i></a> <a href="#" data-id="' + e.id + '" class="event-del"><i class="fa fa-trash-o fa-1_5x"></i></a>';
                            }
                            segTel += '</span></li>';
                            tbody.append(segTel);
                            didAddDayHeader = true;
                        ...
                        var segEl = $('\
                            <li class="my-fc-row clearfix" data-club="' + e.club + '" data-loc="' + e.location + '">\
                                <div class="my-fc-title">' + e.description + '</div>\
                                <span class="my-fc-header-date text-center">' + eventStart.format("dd") + ', ' + eventStart.format(this.opt('dateFormat')) + '<br />' + e.club + '</span>\
                            </li>');
                        tbody.append(segEl);
    ...
    fcViews.callistMonth = {
        type: 'callist',
        duration: { months: 1 }
    };
    fcViews.callistUpComing = {
        type: 'callist',
        duration: { weeks: 8 },
        rows: 8
    };
