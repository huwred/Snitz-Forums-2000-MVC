...
namespace PetaPoco
{
	...
	public class Database : IDisposable
	{
		...
		public Database(string connectionStringName)
		{
			...
			if (ConfigurationManager.ConnectionStrings[connectionStringName] != null)
			{
			    ...
			    SchemaName = builder.InitialCatalog;
		...
		{
            ForumTablePrefix = ConfigurationManager.AppSettings["forumTablePrefix"];
            MemberTablePrefix = ConfigurationManager.AppSettings["memberTablePrefix"];
            FilterTablePrefix = ConfigurationManager.AppSettings["filterTablePrefix"];
		    if (!String.IsNullOrWhiteSpace(ConfigurationManager.AppSettings["lowercaseQuery"]))
		    {
		        LowercaseQuery = ConfigurationManager.AppSettings["lowercaseQuery"] == "1";
        ...
        public string FilterTablePrefix { get; set; }
        public bool LowercaseQuery { get; set; }
		...
		public int Execute(string sql, params object[] args)
		{
			...
			{
			    if (LowercaseQuery)
                    sql = LowercaseHelper.LowercaseQuery(sql);
				...
				try
				{
					using (var cmd = CreateCommand(_sharedConnection, sql, args))
					{
					    ...
					    return (val == null || DBNull.Value.Equals(val)) ?
					        default(T) : (T)Convert.ChangeType(val, u ?? typeof(T));
        ...
        public Page<T> Page<T>(long page, long itemsPerPage, Sql sql, string sqlcount)
        {
            string sqlCount, sqlPage;
            object[] args = new object[] {};
            BuildPageQueries<T>((page - 1) * itemsPerPage, itemsPerPage, sql.SQL, ref args, out sqlCount, out sqlPage);
            return Page<T>(page, itemsPerPage, sqlcount ?? sqlCount, args, sqlPage, args);
        }
        /// <summary>
        /// Retrieves a page of records	and the total number of available records
        /// </summary>
				...
				try
				{
					using (var cmd = CreateCommand(_sharedConnection, ""))
					{
						...
						cmd.CommandText = string.Format("INSERT INTO {0} ({1}){2} VALUES ({3})",
								_dbType.EscapeTableName(LowercaseQuery ? tableName.ToLower() : tableName),
								string.Join(",", LowercaseQuery ? names.Select(n=>n.ToLower()).ToArray() : names.ToArray()),
								outputClause,
					...
					using (var cmd = CreateCommand(_sharedConnection, ""))
					{
						...
						if (columns == null)
						{
							foreach (var i in pd.Columns)
							{
								...
								if (index > 0)
									sb.Append(", ");
								sb.AppendFormat("{0} = {1}{2}", _dbType.EscapeSqlIdentifier(LowercaseQuery? i.Key.ToLower(): i.Key), _paramPrefix, index++);
						...
						else
						{
							foreach (var colname in columns)
							{
								...
								if (index > 0)
									sb.Append(", ");
								sb.AppendFormat("{0} = {1}{2}", _dbType.EscapeSqlIdentifier(LowercaseQuery ? colname.ToLower() : colname), _paramPrefix, index++);
						...
						cmd.CommandText = string.Format("UPDATE {0} SET {1} WHERE {2} = {3}{4}",
											_dbType.EscapeTableName(LowercaseQuery? tableName.ToLower(): tableName), 
                                            sb.ToString(), _dbType.EscapeSqlIdentifier(primaryKeyName), _paramPrefix, index++);
		...
		{
            var pa = PocoData.ForType(poco.GetType());
		    if (!pa.TableInfo.AutoIncrement)
		        return false;
        ...
        public static string DBTypeName { get; set; }

        public string CharSet()
        {
            //SELECT default_character_set_name FROM information_schema.SCHEMATA WHERE schema_name = "SnitzTest";
            return SingleOrDefault<string>("SELECT default_character_set_name FROM information_schema.SCHEMATA WHERE schema_name =\"" + SchemaName + "\"");
        }
        #endregion
	...
	*/
	...
	public class TableNameAttribute : Attribute
	{
        ...
        public Extras.TablePrefixTypes prefixType { get; set; }
	}
	...
	public static class Mappers
	{
		...
		static void RegisterInternal(object typeOrAssembly, IMapper mapper)
		{
			...
			try
			{
                if (!_mappers.ContainsKey(typeOrAssembly))
	...
	public class TableInfo
	{
		...
		public static TableInfo FromPoco(Type t)
		{
		    ...
		    if (a.Length > 0 )
		    {
                switch ((a[0] as TableNameAttribute).prefixType)
		        {
                    case Extras.TablePrefixTypes.None:
                        //do nothing
                        break;
                    case Extras.TablePrefixTypes.Forum:
                        ti.TableName = ConfigurationManager.AppSettings["forumTablePrefix"] + ti.TableName;
		                break;
                    case Extras.TablePrefixTypes.Member:
                        ti.TableName = ConfigurationManager.AppSettings["memberTablePrefix"] + ti.TableName;
		                break;
                    case Extras.TablePrefixTypes.Filter:
                        ti.TableName = ConfigurationManager.AppSettings["filterTablePrefix"] + ti.TableName;
		                break;
                    case Extras.TablePrefixTypes.Custom:
                        ti.TableName = ConfigurationManager.AppSettings["customTablePrefix"] + ti.TableName;
                        break;
                    default:
		                break;
		        }
	...
	namespace Internal
	{
		...
		internal class PocoColumn
		{
		    ...
		    public virtual object ChangeType(object val)
		    {
		        try
		        {
                    var test = Convert.ChangeType(val, PropertyInfo.PropertyType);
		        }
		        catch (Exception)
		        {
                    throw new Exception("Petapoco:3467 " + PropertyInfo.Name);
		        }
		...
		class PocoData
		{
			public static PocoData ForObject(object o, string primaryKeyName)
			{
				...
				if (t == typeof(System.Dynamic.ExpandoObject))
				{
					...
					{
						if (col != primaryKeyName && !pd.Columns.ContainsKey(col))
						...
						return delegate(object src)
						       {
                                   try
                                   {
                                       var test = Convert.ChangeType(src, typeof(int), null);
                                   }
                                   catch (Exception)
                                   {
                                       throw new Exception("Petapoco:3794");
                                   }
                                   return Convert.ChangeType(src, typeof(int), null);
						       };
					}
				}
						...
						return delegate(object src)
						       {
                                   try
                                   {
                                       Type t = Nullable.GetUnderlyingType(srcType) ?? srcType;


                                       return (src == null) ? null : Convert.ChangeType(src, t);
                                   }
                                   catch (Exception ex)
                                   {
                                       throw new Exception("Petapoco:3816: " + pc.ColumnName + " : " + ex.Message);
                                   }
						       };
					}
				}
        ...
        static class LowercaseHelper
        {
            public static string LowercaseQuery(string sql)
            {
                if (sql.StartsWith(";"))
                    return sql.Substring(1);

                foreach (Match match in rxParse.Matches(sql))
                {
                    sql = Regex.Replace(sql,Regex.Escape(match.Value), match.Value.ToLower());
                }
                return sql;
            }

            static Regex rxParse = new Regex(@"(?<unquoted>[^""'\s]+)|(?:[""][^""]+?[""])|(?:['][^ ']+?['])", RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.IgnoreCase | RegexOptions.Multiline);
		...
		internal static class EnumMapper
		{
			public static object EnumFromString(Type enumType, string value)
			{
				Dictionary<string, object> map = _types.Get(enumType, () =>
				{
					...
					{
                        if(!newmap.ContainsKey(v.ToString()))
		...
		internal static class PagingHelper
		{
			...
			public static bool SplitSQL(string sql, out SQLParts parts)
			{
			    ...
			    m = rxCollate.Match(parts.sqlCount);
			    if (!m.Success)
			    {
                    parts.sqlCollate = null;
			    }
			    else
			    {
                    g = m.Groups[0];
                    parts.sqlOrderBy += g.ToString();
                    parts.sqlCount = rxCollate.Replace(parts.sqlCount, ""); 
                    parts.sqlSelectRemoved = rxCollate.Replace(parts.sqlSelectRemoved, "");
            ...
            public static Regex rxCollate = new Regex(@"\s*COLLATE\s+\S+\s*\S*");
